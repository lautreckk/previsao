      ...
      <header className="bg-[#0b1120]/80 backdrop-blur-xl fixed top-0 left-0 right-0 z-50 shadow-2xl shadow-emerald-500/10 overflow-hidden">
        <div className="flex justify-between items-center px-4 h-16">
          <div className="flex items-center gap-3">
            <button onClick={() => router.back()}><span className="material-symbols-outlined text-[#80FF00]">arrow_back</span></button>
          ...
          <div className="bg-surface-container-highest px-4 py-1.5 rounded-full border border-white/10 flex items-center gap-2">
            ...
            <span className="font-headline font-bold text-[#80FF00]">R$ {user.balance.toFixed(2)}</span>
          </div>
        </div>
      </header>
          ...
          {(["ativas", "resolvidas", "canceladas"] as const).map((tab) => (
            <button
              ...
              className={`px-6 py-2 rounded-full font-headline font-bold text-sm whitespace-nowrap transition-all ${
                activeTab === tab
                  ? "bg-[#80FF00]/10 text-[#80FF00] shadow-[0_0_15px_rgba(128,255,0,0.2)]"
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-end">
            ...
            {activeTab === "ativas" && activeBets.length > 0 && (
              <span className="text-[10px] font-bold text-[#80FF00] uppercase tracking-[0.2em]">Live Tracking</span>
            )}
          </div>
          ...
          {displayedBets.length === 0 ? (
            <div className="bg-surface-container rounded-2xl p-8 text-center">
              ...
              <Link href="/" className="text-[#80FF00] text-sm font-bold font-headline mt-2 inline-block">Explorar mercados</Link>
            </div>
          ) : (
            displayedBets.map((bet) => (
              <div key={bet.id} className={`bg-surface-container rounded-2xl overflow-hidden relative ${bet.status === "pending" ? "border-l-4 border-[#80FF00]" : ""}`}>
                <div className="p-5 space-y-4">
                  <div className="flex justify-between items-start">
                    <div className="space-y-1 min-w-0 flex-1 mr-3">
                      {bet.status === "pending" && (
                        <div className="flex items-center gap-2">
                          <span className="flex h-2 w-2 rounded-full bg-[#80FF00] animate-pulse" />
                          <p className="text-[10px] font-black text-[#80FF00] uppercase tracking-widest">Pendente</p>
                        </div>
                      )}
                    ...
                    <div className="bg-surface-container-highest p-3 rounded-2xl text-center min-w-[70px] shrink-0">
                      ...
                      <p className="font-headline font-black text-[#A0FF40] text-lg">{bet.odds.toFixed(2)}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    <div className="space-y-1">
                      <p className={`text-[10px] font-black uppercase tracking-widest ${bet.status === "won" ? "text-[#80FF00]" : bet.status === "lost" ? "text-error" : "text-[#80FF00]"}`}>
                        {bet.status === "pending" ? "Retorno Potencial" : bet.status === "won" ? "Ganho" : "Perdido"}
                      </p>
                      <p className={`font-headline font-black text-3xl tracking-tighter ${bet.status === "won" ? "text-[#80FF00]" : bet.status === "lost" ? "text-error" : "text-[#80FF00]"}`}>
