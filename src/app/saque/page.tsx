  ...
  return (
    <div className="min-h-screen bg-surface-dim text-on-surface overflow-x-hidden w-full max-w-[100vw]">
      <div className="fixed top-0 left-0 right-0 z-50 bg-[#0b1120]/80 backdrop-blur-xl bg-gradient-to-b from-[#0f1729] to-transparent shadow-2xl shadow-emerald-500/10 flex items-center px-4 h-16 overflow-hidden">
        <button onClick={() => step === "confirm" ? setStep("form") : router.back()} className="text-[#80FF00] mr-4"><span className="material-symbols-outlined">arrow_back</span></button>
        <h1 className="text-base font-bold font-headline uppercase tracking-tight flex-1">Sacar via PIX</h1>
        ...
        {step === "form" && (
          <div className="flex flex-col gap-4 animate-fade-in-up">
            ...
            <div className="rounded-2xl border border-white/5 overflow-hidden" style={{ background: `linear-gradient(135deg, ${currentTier.color}15, ${currentTier.color}05)` }}>
              <div className="p-5">
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ background: `${currentTier.color}25` }}>
                    <span className="material-symbols-outlined text-2xl" style={{ color: currentTier.color, fontVariationSettings: "'FILL' 1" }}>{currentTier.icon}</span>
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-lg font-black font-headline" style={{ color: currentTier.color }}>Nivel {userLevel}</span>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-white/5 rounded-xl p-3 text-center">
                    <span className="material-symbols-outlined text-base mb-1 block" style={{ color: currentTier.color }}>speed</span>
                    <p className="text-[10px] text-on-surface-variant">Tempo de saque</p>
                    <p className="text-sm font-black font-headline" style={{ color: currentTier.color }}>{currentTier.withdrawTime}</p>
                  </div>
                  <div className="bg-white/5 rounded-xl p-3 text-center">
                    <span className="material-symbols-outlined text-base mb-1 block" style={{ color: currentTier.color }}>account_balance</span>
                    <p className="text-[10px] text-on-surface-variant">Limite por saque</p>
                    <p className="text-sm font-black font-headline" style={{ color: currentTier.color }}>R$ {currentTier.maxWithdraw.toLocaleString("pt-BR")}</p>
                  </div>
                  <div className="bg-white/5 rounded-xl p-3 text-center">
                    <span className="material-symbols-outlined text-base mb-1 block" style={{ color: currentTier.color }}>redeem</span>
                    <p className="text-[10px] text-on-surface-variant">Bonus diario</p>
                    <p className="text-sm font-black font-headline" style={{ color: currentTier.color }}>{currentTier.dailyBonus > 0 ? `R$ ${currentTier.dailyBonus.toFixed(2)}` : "---"}</p>
                  </div>
                  <div className="bg-white/5 rounded-xl p-3 text-center">
                    <span className="material-symbols-outlined text-base mb-1 block" style={{ color: currentTier.color }}>trending_up</span>
                    <p className="text-[10px] text-on-surface-variant">Bonus deposito</p>
                    <p className="text-sm font-black font-headline" style={{ color: currentTier.color }}>{currentTier.depositBonus > 0 ? `+${currentTier.depositBonus}%` : "---"}</p>
                  </div>
                </div>
                ...
                {nextTier && (
                  <div className="mt-3 bg-white/5 rounded-xl p-3 flex items-center gap-3">
                    <span className="material-symbols-outlined text-lg" style={{ color: nextTier.color, fontVariationSettings: "'FILL' 1" }}>{nextTier.icon}</span>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-bold">Proximo: <span style={{ color: nextTier.color }}>{nextTier.name}</span></p>
                      <p className="text-[10px] text-on-surface-variant">Saque em {nextTier.withdrawTime} + limite R$ {nextTier.maxWithdraw.toLocaleString("pt-BR")} + bonus deposito {nextTier.depositBonus}%</p>
                    </div>
            ...
            <div className="bg-surface-container rounded-2xl p-4 border border-white/5">
              <div className="flex items-center gap-3">
                ...
                <div className="text-right">
                  <p className="text-xs text-on-surface-variant">Disponivel</p>
            ...
            <div className="bg-surface-container rounded-2xl p-5 border border-white/5">
              <h2 className="text-sm font-bold text-on-surface-variant mb-4 uppercase tracking-wider">Quanto deseja sacar?</h2>
              <div className="relative mb-3">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-on-surface-variant text-lg font-bold">R$</span>
                <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00" min={minWithdrawal} className="w-full bg-surface-container-lowest rounded-2xl pl-12 pr-4 py-4 text-white text-2xl font-black outline-none focus:ring-2 focus:ring-[#80FF00]/40 border border-white/5" />
              </div>
            ...
            {/* PIX key type */}
            <div className="bg-surface-container rounded-2xl p-5 border border-white/5">
              <h2 className="text-sm font-bold text-on-surface-variant mb-4 uppercase tracking-wider">Tipo de chave PIX</h2>
              <div className="grid grid-cols-2 gap-2 mb-4">
                {PIX_KEY_TYPES.map((t) => (
                  <button key={t.value} onClick={() => { setPixKeyType(t.value); setPixKey(""); }} className={`py-3 px-3 rounded-2xl text-sm font-bold transition-all active:scale-95 flex items-center justify-center gap-2 ${pixKeyType === t.value ? "kinetic-gradient text-[#0a0a0a] glow-green" : "bg-surface-container-highest text-on-surface hover:bg-surface-bright"}`}>
              ...
              <div className="mb-4">
                <label className="text-xs text-on-surface-variant mb-1.5 block">Chave PIX</label>
                <input
                  ...
                  className="w-full bg-surface-container-lowest rounded-2xl px-4 py-3.5 text-white text-sm outline-none focus:ring-2 focus:ring-[#80FF00]/40 border border-white/5"
                />
              </div>

              <div>
                <label className="text-xs text-on-surface-variant mb-1.5 block">Nome do titular</label>
                <input
                  type="text"
                  value={holderName}
            ...
            {/* Submit */}
            <button onClick={handleConfirm} disabled={!canSubmit} className="w-full py-4 rounded-2xl kinetic-gradient text-[#0a0a0a] font-black font-headline text-base disabled:opacity-40 transition-all hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-2 uppercase tracking-wider glow-green">
            ...
            <div className="bg-surface-container rounded-2xl border border-white/5 overflow-hidden">
              <button onClick={() => setShowAllTiers(!showAllTiers)} className="w-full p-4 flex items-center gap-2 hover:bg-white/5 transition-colors">
                <span className="material-symbols-outlined text-amber-400 text-sm" style={{ fontVariationSettings: "'FILL' 1" }}>emoji_events</span>
                <span className="text-sm font-bold font-headline flex-1 text-left">Programa VIP - Beneficios por Nivel</span>
            ...
            <div className="bg-surface-container rounded-2xl p-4 border border-white/5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-bold font-headline">Prazo de Processamento</span>
              </div>
              <p className="text-xs text-on-surface-variant">
                Seu nivel <span className="font-bold" style={{ color: currentTier.color }}>{currentTier.name}</span> tem prazo de processamento de <span className="font-bold text-white">{currentTier.withdrawTime}</span>.
